...
public class Employee implements Serializable{
	...
	private Boolean provisioning;
		...
		private boolean stopPushNotificationsAfterWorkingHours = false;
	...
	public double getDistanceFormJobToDisp() {
		return Api.round(distanceFormJob/1000, 3);
	}
	...
	public boolean isSame(Employee employee) {
		if(employee.getEmpId() != getEmpId()){
			return false;
		}
		if(!Api.isEqualString(employee.getClientEmpId(), getClientEmpId(), true)){
			return false;
		}
		if(employee.getCompanyId() != getCompanyId()){
			return false;
		}
		if(!Api.isEqualLong(employee.getCalendarId(), getCalendarId())){
			return false;
		}
		if(!Api.isEqualString(employee.getEmpNo(), getEmpNo(), true)){
			return false;
		}
		if(!Api.isEqualString(employee.getEmpFirstName(), getEmpFirstName(), true)){
			return false;
		}
		if(!Api.isEqualString(employee.getEmpLastName(), getEmpLastName(), true)){
			return false;
		}
		if(employee.getEmpTypeId() != getEmpTypeId()){
			return false;
		}
		if(!Api.isEqualString(employee.getEmpPhone(), getEmpPhone(), true)){
			return false;
		}
		if(!Api.isEqualString(employee.getEmpAddressStreet(), getEmpAddressStreet(), true)){
			return false;
		}
		if(!Api.isEqualString(employee.getEmpAddressArea(), getEmpAddressArea(), true)){
			return false;
		}
		if(!Api.isEqualString(employee.getEmpAddressCity(), getEmpAddressCity(), true)){
			return false;
		}
		if(!Api.isEqualString(employee.getEmpAddressPincode(), getEmpAddressPincode(), true)){
			return false;
		}
		if(!Api.isEqualString(employee.getEmpAddressLandMark(), getEmpAddressLandMark(), true)){
			return false;
		}
		if(!Api.isEqualString(employee.getEmpAddressState(), getEmpAddressState(), true)){
			return false;
		}
		if(!Api.isEqualString(employee.getEmpAddressCountry(), getEmpAddressCountry(), true)){
			return false;
		}
		if(!Api.isEqualString(employee.getEmpEmail(), getEmpEmail(), true)){
			return false;
		}
		if(!Api.isEqualString(employee.getImei(), getImei(), true)){
			return false;
		}
		if(!Api.isEqualString(employee.getHomeLat(), getHomeLat(), true)){
			return false;
		}
		if(!Api.isEqualString(employee.getHomeLong(), getHomeLong(), true)){
			return false;
		}
		if(!Api.isEqualString(employee.getWorkLat(), getWorkLat(), true)){
			return false;
		}
		if(!Api.isEqualString(employee.getWorkLong(), getWorkLong(), true)){
			return false;
		}
		if(!Api.isEqualLong(employee.getManagerId(), getManagerId())){
			return false;
		}
		if(employee.isManager() != isManager()){
			return false;
		}
//		if(employee.getStatus() != getStatus()){
//			return false;
//		}
		return true;
	}
	...
	public String getLastSignoutTime() {
		if(lastSignoutTime == null){
			lastSignoutTime = "";
		}
		lastSignoutTime = Api.removeTrailingZeroFromDateTime(lastSignoutTime);
	...
	public boolean isSameForLTFS(Employee employee) {
		if(employee.getEmpId() != getEmpId()){
			return false;
		}
		if(!Api.isEqualString(employee.getClientEmpId(), getClientEmpId(), true)){
		...
		if(employee.getTimezoneId().intValue() != getTimezoneId().intValue()){
			return false;
		}
		if(!employee.getDesignation().equals(getDesignation())){
			return false;
		}
		if(employee.getRank() != getRank()){
			return false;
		}
		if(!Api.isEqualString(employee.getSkillEntityIds(),getSkillEntityIds())){
			return false;
		}
		if(!Api.isEqualString(employee.getEmpAddressDistrict(),getEmpAddressDistrict())){
			return false;
		}
		if(!Api.isEqualString(employee.getEmpJoiningDate(),getEmpJoiningDate())){
			return false;
		}
		if(!Api.isEqualString(employee.getEmpProhibitionPeriod(),getEmpProhibitionPeriod())){
			return false;
		}
		if(!Api.isEqualString(employee.getExpiryDate(),getExpiryDate())){
			return false;
		}
		return true;
	}
